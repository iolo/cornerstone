...
/** @deprecated in favor of CornerstoneClient */
export class Client {
   ...
   * @param site
   */
  getStub(taskName: string, environment: string, site: string): <T>(message: T) => string {
    return <T>(message: T): string => {
      ...
      this.logger.debug('cornerstone task requested with message: %s', publishingMessageString);
  ...
  private static getTopicName(taskName: string, environment: string, site: string) {
